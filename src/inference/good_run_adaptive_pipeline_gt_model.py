...
def run_adaptive_inference_for_sequence(
    ...
    for idx, curr_img_path in enumerate(tqdm(image_paths, desc=f"Adaptive {sequence_dir.name}")):
        frame_id = int(curr_img_path.stem)

        if idx == 0:
            detections = run_yolo_on_frame(
                yolo_model, curr_img_path, confidence_thresh=yolo_conf_thresh
            )
        ...
        decision_rows.append(
            {
                ...
                "skip_streak_before": skip_streak if yolo_ran == 0 else 0,
                "num_detections": len(detections),
            }
        )
    ...
    print(f"Saved detections to {detections_output_csv}")


def main():
    base_dir = Path("data/raw/MOT17/train")
    model_path = Path("outputs/models/random_forest_gt/random_forest_gt_model.joblib")
